    ...
    return (
        <main className="h-screen">
            ...
            <div className="relative flex justify-center w-full max-h-screen pb-24">
                ...
                <div className="fixed bottom-0 w-full flex flex-col items-center pb-10">
                    <div className="fixed bottom-0 cursor-text md:flex w-1/2 mx-auto justify-evenly items-center playfair-display-600 p-2 my-16 md:p-5 bg-linear-to-tr from-teal-800 via-sky-900 to-green-800 rounded-full z-20">
                        ...
                        <input value={input} onChange={(e) => setInput(e.target.value)} onSubmit={handleSubmitMessage} onKeyDown={ (e) => { if (e.key === "Enter" && input.length > 0 && exceededRateLimit === false) handleSubmitMessage() } } type="text" className="w-full h-full outline-0 font-light text-xl text-stone-200 px-2" placeholder="Ask me anything about work, goals, or background 💬" />
                    ...
                    <p className="text-xs text-stone-300">Please note, my digital twin can make mistakes. Reach out via LinkedIn if necessary.</p>
                </div>
            </div>
        </main>
    )
